...
# (기존의 함수들: load_data_and_models, get_nlp_recommendations, get_content_recommendations, get_collaborative_recommendations, ensemble_recommendations)
@st.cache_resource
...
def get_collaborative_recommendations(game_name, top_n=5):
    ...
    # 예측 점수로 정렬하고 상위 N개 선택
    top_indices = np.argsort(predictions)[::-1][1:top_n+1]  # 자기 자신 제외
...
if search_method == '게임 이름':
    ...
    if game_name:
        ...
        if not nlp_recommendations.empty:
            ...
            st.write("이 추천 목록은 NLP 기반, 콘텐츠 기반, 협업 필터링 기반 추천 시스템의 결과를 종합하여 만들어졌습니다.")
        else:
